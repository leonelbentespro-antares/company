...
import { getThreatReport, getBlockedIPs, unblockIP, getThreatScore, } from '../middleware/threatDetector.js';
...
adminRouter.get('/security/threats', (req, res) => {
...
adminRouter.get('/security/ip/:ip', (req, res) => {
    const ip = String(req.params['ip'] ?? '');
    const entry = getThreatScore(ip);
    if (!entry) {
        res.json({ ip, status: 'clean', score: 0 });
        return;
    }
    res.json({
        ip,
...
adminRouter.delete('/security/ip/:ip/block', (req, res) => {
    const ip = String(req.params['ip'] ?? '');
...
adminRouter.get('/security/health', (_req, res) => {
    res.json({
        status: 'ok',
