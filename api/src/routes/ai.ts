...
const ALLOWED_DOC_TYPES = ['RESUMO_PROCESSO', 'PETICAO_INICIAL', 'CONTRATO', 'PARECER'] as const;
type AllowedDocType = typeof ALLOWED_DOC_TYPES[number];

aiRouter.post('/documents/generate', async (req: Request, res: Response) => {
    const tenantId = req.tenantId!;
    const userId   = req.userId!;
    const { type, context } = req.body as { type: string; context?: unknown };

    if (!type || !ALLOWED_DOC_TYPES.includes(type as AllowedDocType)) {
        res.status(400).json({
...
aiRouter.get('/documents/status/:jobId', async (req: Request, res: Response) => {
    const jobId = String(req.params['jobId'] ?? '');
    try {
        const job = await documentGenerationQueue.getJob(jobId ?? '');
        if (!job) { res.status(404).json({ error: 'Job não encontrado.', code: 'JOB_NOT_FOUND' }); return; }
